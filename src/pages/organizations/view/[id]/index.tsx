...
import { deleteModelById } from 'apiSdk/models';
...
function OrganizationViewPage() {
  ...
  const modelHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('model', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/models/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.model?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/models/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>
